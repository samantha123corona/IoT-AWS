...
export default function MonitorObstacles({ obstacles }) {
  return (
    <div className="card shadow-sm h-100">
      <div className="card-header bg-danger text-white fw-semibold d-flex justify-content-between align-items-center">
        <span>Últimos 10 obstáculos detectados</span>
        <small className="badge bg-light text-dark">
          {obstacles.length} registros
        </small>
      </div>
      <div className="card-body p-0">
        <div className="table-responsive">
              ...
              {obstacles?.length ? (
                obstacles.map((o) => (
                  <tr key={o.id}>
                    <td>{o.id}</td>
                    <td>
                      <strong>{o.distancia_obstaculo}</strong> cm
                    </td>
                    <td>{o.fecha_registro ?? o.created_at ?? "—"}</td>
                  </tr>
                ))
